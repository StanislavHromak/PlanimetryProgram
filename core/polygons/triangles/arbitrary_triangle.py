        ...
        if not self._is_target("side") and not self._is_target("angles"):
            self._add_step(
                ...
                rule="Теорема косинусів: квадрат будь-якої сторони трикутника дорівнює сумі "
                     "квадратів двох інших сторін мінус подвоєний добуток цих сторін на косинус кута між ними.",
    ...
    def _calculate(self):
        ...
        if self.task_type == "SSS":
            ...
            if self._is_target("angles"):
                ...
                result["angle_b"] = self._add_step(
                    f"Крок {self.step_num}. Знаходимо кут β",
                ...
                result["angle_c"] = self._add_step(
                    f"Крок {self.step_num}. Знаходимо кут γ",
                    "γ = 180° - α - β",
            ...
            self._add_info(f"Довільний трикутник (SAS): a={self.a}, b={self.b}, γ={self.angle_c}°")
            self.c = self._compute_side_c_sas()

            if self._is_target("side"):
                result["side_c"] = self._add_step(
                    f"Крок {self.step_num}. Знаходимо сторону c",
                    "c = √(a² + b² - 2·a·b·cos(γ))",
                    f"c = √({self.a}² + {self.b}² - 2·{self.a}·{self.b}·cos({self.angle_c}°))",
                    self.c,
                    rule="Теорема косинусів: квадрат сторони дорівнює сумі квадратів двох інших мінус їх подвоєний добуток на косинус кута між ними."
                )
                self.step_num += 1
            ...
            self._add_info(f"Довільний трикутник (ASA): a={self.a}, β={self.angle_b}°, γ={self.angle_c}°")
            if self._is_target("side"):
                self.angle_a = 180 - self.angle_b - self.angle_c
                rad_a = math.radians(self.angle_a)
                rad_b = math.radians(self.angle_b)
                ...
                result["side_b"] = self._add_step(
                    f"Крок {self.step_num}. Знаходимо сторону b",
                    "b = a · sin(β) / sin(α)",
                    f"b = {self.a} · sin({self.angle_b}°) / sin({self.angle_a}°)",
                    self.b,
                    rule="Теорема синусів: сторони трикутника пропорційні синусам протилежних кутів."
        ...
        if self._is_target("area"):
            ...
            result["area"] = self._add_step(
                f"Крок {self.step_num}. Знаходимо площу (формула Герона)",
                "S = √(p·(p-a)·(p-b)·(p-c))",
                f"S = √({p:.2f}·({p:.2f}-{self.a:.2f})·({p:.2f}-{self.b:.2f})·({p:.2f}-{self.c:.2f}))",
                area,
        ...
        if self._is_target("incircle"):
            ...
            result["r_inscribed"] = self._add_step(
                ...
                f"r = {area:.2f} / {p:.2f}",
                area / p,
                rule="Радіус вписаного кола трикутника: r = S / p, де S — площа, p — напівпериметр."
            )
            self.step_num += 1
